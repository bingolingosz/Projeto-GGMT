<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Perfil </title>
    <link rel="stylesheet" href="css/sitefeito.css">
    <link rel="stylesheet" href="css/headerfooter.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>
<body>
    <!-- HEADER -->
    <div class="principal">
        <a href="inicio"><img src="images/logoskayhub.png" alt="logo" width="200px"></a>

        <form action="" class="busca">
            <div class="pesquisar">
                <input type="text" placeholder="Pesquisar">
            </div>
        </form>

        <div class="icone">
            <a href="conecte"><i class="fa-solid fa-user"></i></a>
            <div class="iconeuser">
                <p>Olá, <a href="conecte">ENTRE</a> ou <br>
                    <a href="registro">CADASTRE-SE</a>
                </p>
            </div>
            <a href="#"><i class="fa-solid fa-cloud-arrow-up"></i></a>
        </div>
    </div>

    <header>
       <nav>
         <ul>
            <li><a href="inicio">INICIO</a></li>
            <li>|</li>
            <li><a href="sitespublicados">SITES</a></li>
            <li>|</li>
            <li><a href="indie">JOGOS</a></li>
            <li>|</li>
            <li><a href="sobrenos">SOBRE NÓS</a></li>
         </ul>
        </nav>
    </header>

    <!-- CONTEÚDO -->
    <section>
        <div class="container-banner">
            <div class="container-banner2">
                <figure>
                    <img src="images/homem.jpg" alt="Exemplo de imagem">
                </figure>
                <a href="perfil">Renato</a>
            </div>
            <hr>

            <div class="descricao-site">
                <div class="descricao"><p>Descrição do site:</p></div>
                <p>Esse é um site que eu fiz com o tema de Dead by Daylight (DBD), focado em explicar as mecânicas, personagens e modos do jogo. Aqui, você encontra guias sobre como jogar tanto com os sobreviventes quanto com o assassino, além de dicas para melhorar suas estratégias. O design foi inspirado no clima sombrio do jogo, tornando a navegação tanto informativa quanto imersiva. O objetivo é ajudar jogadores, de iniciantes a veteranos, a entender melhor o jogo e aprimorar suas habilidades!</p>
            </div>

        </div>
        <div class="conteudo">
            <iframe src="htmls/sitesFeitos/dbd/site dbd.html"></iframe>
        </div>
    </section>

   <!-- FOOTER -->
    <footer>
        <ul>
            <li class="icons"><a href="#"><img src="images/icon instagram.png" alt="Instagram" width="30px"></a></li>
            <li class="icons"><a href="#"><img src="images/icon twitter.png" alt="Twitter" width="30px"></a></li>
            <li class="icons"><a href="#"><img src="images/icon linkedin.png" alt="linkedin" width="30px"></a></li>
            <li class="icons"><a href="#"><img src="images/icon github.png" alt="Github" width="30px"></a></li>
        </ul>

        <ul>
            <li><a href="inicio">Inicio</a></li>
            <li>|</li>
            <li><a href="equipe">Equipe</a></li>
            <li>|</li>
            <li><a href="suporte">Suporte</a></li>
        </ul>
        <p> Copyright &copy;2025 SkayHub. Todos os direitos reservados.</p>
    </footer>
</body>
</html>
